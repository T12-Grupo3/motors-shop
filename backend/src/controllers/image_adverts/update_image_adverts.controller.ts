...
const updateImageAdvertsController = async (req: Request, res: Response) => {
    const {image_adverts, description_image }: IImageAdvertsRequest = req.body
    const imageAdvertId: string = req.params.id
    
            
    const updatedImageAdvert = await updateImageAdvertsService({image_adverts, description_image}, imageAdvertId)
