...
const UserProvider = ({ children }: iUserProvider) => {
  ...
  const api_create_user = async (data: iUserRequest) => {
    try {
      ...
      toast.success("Seu cadastro foi feito com sucesso, faço o login", {
        autoClose: 1000,
      });
    } catch (error) {
      toast.error(
        "Ocorreu algum erro ao finalizar seu cadastro, verifique se todos os campos estão preenchidos corretamente."
      );
      console.error(error);
    }
  };
  ...
  const api_update_user = async (data: iUserUpdate) => {
    try {
      ...
      toast.error(
        "Ocorreu algum erro ao editar seu perfil, verifique se todos os campos foram preenchidos corretamente"
      );
      console.error(error);
    }
  };
  ...
  const api_update_address = async (data: iAdressRequest) => {
    try {
      const res = await api.patch(`/users/${userId}`, data);
      toast.success("Seu endreço foi editado com sucesso!", {
        autoClose: 1000,
      });
    } catch (error) {
      toast.error(
        "Ocorreu algum erro ao editar seu endereço, verifique se todos os campos foram preenchidos corretamente"
      );
      console.error(error);
    }
  };
  ...
  const api_delete_user = async (user_id: string) => {
    try {
      ...
      toast.success(
        "Seu perfil foi excluído, sentiremos sua falta, volte logo :(",
        { autoClose: 1000 }
      );
      logoutProfileView();
    } catch (error) {
      toast.error(
        "Ocorreu algum erro ao deletar seu perfil, atualize a página e tenta novamente"
      );
      console.error(error);
    }
  };
  ...
  const api_change_password = async (data: iPasswordChangeRequest) => {
    try {
      ...
      toast.success("Sua senha foi alterada, faça o login", {
        autoClose: 1000,
      });
    } catch (error) {
      toast.error(
        "Ocorreu algum erro ao alterar sua senha, verifique se seu email está correto"
      );
      console.error(error);
    }
  };
  ...
  const logoutProfileView = () => {
    ...
    localStorage.clear();
    setIsLogged(false);
    toast.warning("Você foi deslogado, faça login novamente", {
      autoClose: 1000,
    });
    navigate("/home", { replace: true });
